...
const LOBBY_TIMEOUT_MS = Number(
    process.env.LOBBY_TIMEOUT_MS ?? 1000 * 60 * 30
);
...
function removeSocketFromAllLobbies(
    socket: Socket,
    exceptPartyId?: string
) {
    for (const [partyId, lobby] of lobbyMap.entries()) {
        if (exceptPartyId && partyId === exceptPartyId) {
            continue;
        }
...
setInterval(() => {
    ...
    for (const [partyId, lobby] of lobbyMap.entries()) {
        if (
            now - lobby.lastActivity >
            LOBBY_TIMEOUT_MS
        ) {
            console.log(
                `Destroying inactive lobby ${partyId}`
            );
...
io.on("connection", (socket) => {
    ...
    socket.on("latency", (ping) => {
        const summonerId =
            socketToSumIdMap.get(socket.id);

        if (!summonerId) return;

        for (const lobby of lobbyMap.values()) {
            const member =
                lobby.getMember(socket.id);

            if (!member) continue;

            lobby.touch();

            socket.to(lobby.partyId).emit(
                "latency",
                {
                    id: summonerId,
                    ping,
    ...
    socket.on("party-id", (partyId: string) => {
        ...
        if (!summonerId) {
            console.log(
                "Socket tried joining without summonerId"
            );
